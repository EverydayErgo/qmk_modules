...
Now you can add this module to your keyboard. Drop a **keymap.json** file where your layout is located and include this module. [Read more about community modules.](https://docs.qmk.fm/features/community_modules)
...
```C
  ...
  EIM_ENCODERS_END
```
## Processing encoder events
Based on your preference there are two ways to handle what action is performed when rotating the encoder.

1) **Keymap based**

   If you don't need a fine control over every encoders direction and resolution you can use this method. Simply in your layout at the location of **Pin A** and **Pin B** put a keycode that you would like to be triggered when encoder is rotated. If the direction is not right, just flip the keycodes around. This method doesn't take into account resolution and direction from the encoder mapping table. You will have to rely on the global resolution define and swap keycodes if the direction is not right.
   ```C
...
2) **Callback based**
   ...
   Enable callback based event handling by adding
   ```C
   #define EIM_CALLBACK
   ```
   It's not necessary when callback in enabled but to avoid confusion you might want to put in your layout **KC_NO** at the location where **Pin A** and **Pin B** are connected to the matrix.
   ```C
    const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
        [0] = LAYOUT_ortho_3x3(
            KC_A,    KC_B,    KC_C,
   ...
   ```
   Then you have to handle messages coming from encoders. Implement callback like this
   ```C
...
```
Redefinition must happen before
```C
  EIM_ENCODERS_BEGIN
```
